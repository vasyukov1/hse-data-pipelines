...
kubectl apply -n "${NAMESPACE}" -f "${HW11_ROOT}/demo/coursework-demo-server.yaml"
if ! kubectl rollout status deployment/coursework-demo-server -n "${NAMESPACE}" --timeout="${DEMO_WAIT_TIMEOUT}"; then
  print_debug_info
  kubectl describe deployment coursework-demo-server -n "${NAMESPACE}" || true
  echo
  kubectl describe pods -n "${NAMESPACE}" -l app=coursework-demo-server || true
  exit 1
fi
...
echo
kubectl get svc -n "${NAMESPACE}" coursework-demo-server argo-server minio
